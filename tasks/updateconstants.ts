...
const heroDataUrlsRussian: string[] = []
...
async function start() {
  ...
  ids.forEach((key) => {
    heroDataUrls.push('http://www.dota2.com/datafeed/herodata?language=english&hero_id=' + key)
    heroDataUrlsRussian.push(
      'http://www.dota2.com/datafeed/herodata?language=russian&hero_id=' + key
    )
  })
  let names = Object.keys(heroesVdf.DOTAHeroes).filter((name) => !badNames.has(name))
  names.forEach((name) => {
  ...
  const sources = [
    {
      ...
      transform: (respObj) => {
        ...
        Object.keys(scripts)
          .filter((key) => {
            return (
              !(key.includes('item_recipe') && scripts[key].ItemCost === '0') && key !== 'Version'
            )
          })
          .forEach((key) => {
            ...
            item.dname =
              strings[`DOTA_Tooltip_ability_${key}`] || strings[`DOTA_Tooltip_ability_${key}:n`]
            ...
            item.dmg_type = formatBehavior(scripts[key].AbilityUnitDamageType) || undefined
            item.bkbpierce = formatBehavior(scripts[key].SpellImmunityType) || undefined
            item.dispellable = formatBehavior(scripts[key].SpellDispellableType) || undefined
            item.target_team = formatBehavior(scripts[key].AbilityUnitTargetTeam) || undefined
            item.target_type = formatBehavior(scripts[key].AbilityUnitTargetType) || undefined
                    ...
                    if (tooltipKey in strings) {
                      if (string.includes('$') || /[a-z]/.test(string)) {
                        ...
                        display = string.replace(
                          ...
                          (str, pct = '', pm = '', variable) =>
                            `${pm} {value}${pct} ` +
                            (strings[`dota_ability_variable_${variable?.replace('$', '')}`] || '')
                        )
                      }
                      if (!/[a-z]/.test(string)) {
                        // Upper case stats, where the number is displayed in the end
                        display = string.replace(/^(%)?(.+)/, (str, pct = '', rest) => {
      ...
      transform: (respObj) => {
        ...
        Object.keys(scripts)
          ...
          .forEach((key) => {
            ...
            ability.dispellable = formatBehavior(scripts[key].SpellDispellableType) || undefined
            ability.target_team = formatBehavior(scripts[key].AbilityUnitTargetTeam) || undefined
            ability.target_type = formatBehavior(scripts[key].AbilityUnitTargetType) || undefined
            ...
            ability.attrib = formatAttrib(specialAttr, strings, `DOTA_Tooltip_ability_${key}_`)

            ability.lore = strings[`DOTA_Tooltip_ability_${key}_Lore`]

            const ManaCostKey =
              scripts[key].AbilityManaCost ?? scripts[key].AbilityValues?.AbilityManaCost
            const CooldownKey =
              scripts[key].AbilityCooldown ?? scripts[key].AbilityValues?.AbilityCooldown
                    ...
                    if (isObj(val)) {
                      ...
                      if (key.indexOf('scepter') !== -1 || key.indexOf('shard') !== -1) {
                        const bonus = Object.keys(val)
                          .filter((k) => k !== key)
                          .find((k) => k.indexOf('scepter') !== -1 || k.indexOf('shard') !== -1)
                        if (bonus) {
    ...
    },
    {
      ...
      transform: (respObj) => {
        ...
        const keys = Object.keys(units).filter((name) => {
          ...
          if (
            getUnitProp(unit, 'UnitRelationshipClass') !== 'DOTA_NPC_UNIT_RELATIONSHIP_TYPE_DEFAULT'
          ) {
            return false
          }
    ...
    },
    {
      ...
      transform: (respObj) => {
        let heroes: any = []
        let keys = Object.keys(respObj[1].DOTAHeroes).filter((name) => !badNames.has(name))
        keys.forEach((name) => {
          let h: any = formatVpkHero(name, respObj[1])
          h.localized_name =
            respObj[1].DOTAHeroes[name].workshop_guide_name ?? respObj[0].lang.Tokens[name + ':n']
          heroes.push(h)
        })
    ...
    },
    {
      ...
      transform: (respObj) => {
        ...
        Object.keys(heroes).forEach(function (heroKey) {
          ...
          Object.entries(heroes[heroKey].Facets ?? []).forEach(([key, value]: [string, any], i) => {
            const abilities = Object.values(value.Abilities || {}).map((a: any) => a.AbilityName)
            ...
            if (description.includes('{s:facet_ability_name}')) {
              description = description.replace('{s:facet_ability_name}', title)
            }
    ...
    },
    {
      ...
      url: [heroesUrl, ...abilitiesUrlsRussian],
      transform: (respObj) => {
        const [heroObj, abilityLoc, _, ...heroAbils] = respObj
      ...
      transform: (respObj) => {
        ...
        for (let key of keys) {
          ...
          if (keyArr[0].toLowerCase() == 'general') {
            ...
            let abilityName =
              keyArr.length > 1 ? keyArr.join('_').replace(`${heroName}_`, '') : 'general'
      ...
      transform: (respObj) => {
        ...
        herodata.forEach((hd_hero) => {
          ...
          if (!aghs_element.has_scepter) {
            console.log(
              aghs_element.hero_name + '[' + aghs_element.hero_id + ']' + ": Didn't find a shard..."
            )
          }
  ...
  for (let i = 0; i < sources.length; i++) {
    ...
    fs.writeFileSync('./build/' + s.key + '.json', JSON.stringify(final, null, 2))
  }
...
function calculateValueFromBonus(value, bonus) {
  const rawBonus = bonus.replace('+', '').replace('-', '').replace('x', '').replace('%', '')
  if (value === undefined) {
    return rawBonus
  }
...
function formatAttrib(attributes, strings, strings_prefix) {
  ...
  return (attributes || [])
    ...
    .map((attr) => {
      let key = Object.keys(attr).find((key) => `${strings_prefix}${key.toLowerCase()}` in strings)
      if (!key) {
      ...
      if (match[2]) {
        ...
        if (header.includes('attack_range')) final.footer = final.footer.replace(/<[^>]*>/g, '')
      } else {
      ...
      for (const key of Object.keys(attrib)) {
        ...
        if (isObj(val)) {
          ...
          if (specialBonusKey) {
            ...
            const special = val[specialBonusKey]
            // remove redundant signs
            const specialBonusVal =
              special && typeof special === 'string'
                ? special.replace('+', '').replace('-', '').replace('x', '').replace('%', '')
                : special
  ...
  if (attribs) {
    ...
    template = template.replace(/%([^% ]*)%/g, function (str, name) {
      ...
      if (!attr) {
        ...
        console.log(
          `cant find attribute %${name}% in %${key}% with ${attribs.map((o) => Object.keys(o)[0])}`
        )
        return `%${name}%`
      }
...
function formatBehavior(string) {
  ...
  let split = string
    .split(' | ')
    .filter((item) => !ignoreStrings.has(item.trim()) && extraStrings.hasOwnProperty(item.trim()))
    .map((item) => {
...
function formatVpkHero(key, vpkr) {
  ...
  h.img = '/apps/dota2/images/dota_react/heroes/' + key.replace('npc_dota_hero_', '') + '.png?'
  h.icon =
    '/apps/dota2/images/dota_react/heroes/icons/' + key.replace('npc_dota_hero_', '') + '.png?'
  h.url = vpkrh.url
